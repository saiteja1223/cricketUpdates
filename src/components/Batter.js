...
function Batter({data}) {
  ...
  return (
    <div class="border border-gray-500 mx-5 w-[300px] rounded mix-h-fit">
...
{info && info.slice(0,show?info.length:10).map((item, index) => (
...
<button className='bg-blue-200 m-5 mx-[100px] rounded cursor-pointer' onClick={()=>handleshow()}>{show?'Show Less':'Show All'}</button>
</div>
